...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const row of data) {
      ...
      try {
        ...
        if (!department) {
          ...
          results.skipped++;
          results.errors.push(`Department with ID ${departmentId} not found for penelitian ID ${row.id_data}`);
        ...
        if (!dosen) {
          console.warn(`Dosen not found for research ID ${row.id_data}. Skipping record.`);
          results.skipped++;
          results.errors.push(`Dosen not found for research ID ${row.id_data}`);
        ...
        results.errors.push(`Error processing penelitian ID ${row.id_data}: ${error.message}`);
    ...
    return NextResponse.json({
      ...
      results: {
        ...results,
        summary: `Successfully imported ${results.success} penelitian records. Skipped ${results.skippedNoNIDN} without NIDN, ${results.skippedNoDepartment} without department, and ${results.skippedInvalidFormat} with invalid format.`
      }
    }, { status: 200 });
  } catch (error:any) {
